...
## Theory
В ходе исследования я выяснил, что присутствие нескольких параметров на входе не имеет значения ввиду вида раскрытия формулы в систему:

$$
\begin{align}
(x_1 + x_2 + ... + x_n) \cdot \sum_{i=1}^a w_{1,i} \cdot \sum_{j=1}^b w_{2,j} \cdot ... \sum_{k=1}^c w_{L-1,k} + w_{L,1} \\
(x_1 + x_2 + ... + x_n) \cdot \sum_{i=1}^a w_{1,i} \cdot \sum_{j=1}^b w_{2,j} \cdot ... \sum_{k=1}^c w_{L-1,k} + w_{L,2} \\
... \\
(x_1 + x_2 + ... + x_n) \cdot \sum_{i=1}^a w_{1,i} \cdot \sum_{j=1}^b w_{2,j} \cdot ... \sum_{k=1}^c w_{L-1,k} + w_{L,d}
\end{align}
$$

С условием отсутствия прямого сложения весов из-за присутствия функции активации, ограничивающей возможные сдвиги значений. Это приводит к выводу, что входной слой суммируется, и количество параметров на нём не играет роли.
